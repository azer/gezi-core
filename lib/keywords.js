...
function save (url, options, callback) {
  ...
  options.title && (keywords = keywords.concat(lessCommonWords(anglicize(options.title))));
  options.tags && (keywords = keywords.concat(options.tags.map(anglicize)));
...
function search (keyword, callback) {
  db.query("SELECT * FROM keywords WHERE keyword LIKE '%"+anglicize(keyword)+"%' ORDER BY ts DESC", callback);
}
